...
function buildCSS() {
    ...
    /* ── Button Base (TRANSPARAN — cuma ikon kelihatan) ──
       Ukuran default 36px, tapi BISA SHRINK kalau frame sempit
       (pakai flex-shrink:1 + min-width/height 20px). */
    .jt-btn {
        width: 36px; height: 36px;
        min-width: 20px;             /* batas shrink minimum */
        min-height: 20px;
        max-width: 100%;             /* jangan overflow container */
        max-height: 100%;
        ...
        color: #888;
        outline: none;
        flex-shrink: 1;              /* boleh shrink kalau frame sempit */
        box-sizing: border-box;
    }
    .jt-btn:hover {
    ...
    /* ── Icon (PNG & SVG) — auto-shrink ikut ukuran tombol ── */
    .jt-icon-img {
        width: 100%;                 /* ikut shrink dengan tombol */
        height: 100%;
        max-width: 22px;             /* batas max normal */
        max-height: 22px;
        object-fit: contain;
        pointer-events: none;
